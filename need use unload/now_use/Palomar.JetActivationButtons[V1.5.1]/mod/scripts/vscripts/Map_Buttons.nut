...
void function Map_Buttons_Init(){
    ...
    if (GetMapName() == "mp_thaw") {
        createspawnconsole(< -931.822, 1973.69, -159.969>, <0, 0, 0>)
        createselectpanel(< -855.339, 2036.98, -110.785> ,  <0,-157,270>)
...
void function setbuttonactive(entity button) {
    ...
    button.SetUsePrompts( "Hold %use% " + " to spawn a ship", "Press %use%" + " to spawn a ship" )
    ...
    if (PlayerPosInSolid(player,<landingspot.x,landingspot.y,landingspot.z + 30>) == false &&  buttonsoncooldown == false) {
        ...
        if(storedships != 0){//if there are ships left
            ...
            foreach (player in GetPlayerArray()) {
                if (IsValid(player)) {
                    NSSendLargeMessageToPlayer( player, "A Ship Was Delivered",string(storedships)+" Left In Storage", 3.5, "rui/callsigns/callsign_74_col" )
                }
            }
            ...
            foreach (player in GetPlayerArray()) {
                if (IsValid(player)) {
                    NSSendLargeMessageToPlayer( player, "Last Ship Was Delivered","Wait For More To Finish Production", 3.5, "rui/callsigns/callsign_74_col" )
                }
            }
        }
        ...
        NSSendInfoMessageToPlayer( player, "Landing area is not clear!" )
    ...
    while (true) {
        ...
        if (storedships < 5) {
            if (storedships == 0) {//only send this message when storage was empty before
                ...
                foreach (player in GetPlayerArray()) {
                    if (IsValid(player)) {
                        NSSendLargeMessageToPlayer( player, "A Ship Has Finished Production","It can now be called in at a designated bay.", 3.5, "rui/callsigns/callsign_73_col" )
                    }
                }
            }
            storedships = storedships + 1
...
void function setselectpanelactive(entity button) {
    ...
    button.SetUsePrompts( "Hold %use% " + " to change shipclass", "Press %use%" + " to change shipclass" )
    button.SetUsableByGroup( "pilot" )
    button.SetUsable()
    ...
    if ( selectedtypeid == 1) {selectedtype = "goblin"}
    if ( selectedtypeid == 2) {selectedtype = "crow"}
    if ( selectedtypeid == 3) {selectedtype = "straton"; selectedtypeid = 0}
