...
void function SarahEscortThink( vector origin, entity player )
{
    ...
    Chat_ServerPrivateMessage( player, "[Sarah] follow me", false )
    ...
    if ( !slow )
        Chat_ServerPrivateMessage( player, "[Sarah] We got to the dropship", false )
    ...
    if ( !slow )
    {
        Chat_ServerPrivateMessage( player, "[Sarah] Here, You can have this gift.", false )
        StimPlayer( player, 1000000 )
    }

    wait 2

    Chat_ServerPrivateMessage( player, "[Sarah] Blast off", false )
