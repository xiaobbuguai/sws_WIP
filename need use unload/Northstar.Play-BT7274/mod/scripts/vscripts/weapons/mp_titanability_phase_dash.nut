		...
		if ( PhaseShift( player, 0, shiftTime ) )
		{
			if ( player.IsPlayer() )
			{
				PlayerUsedOffhand( player, weapon )
					...
					float fade = 0.5
					//StatusEffect_AddTimed( soul, eStatusEffect.move_slow, 1.75, shiftTime + fade, fade )
				#elseif CLIENT
...
{
	float movestunEffect = 1.1 - StatusEffect_Get( player, eStatusEffect.dodge_speed_slow )
	float moveSpeed
