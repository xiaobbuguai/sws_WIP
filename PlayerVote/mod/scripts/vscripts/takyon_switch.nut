...
global function SetPlayerModelAndSkins
    ...
    if(!IsLobby() && !IsFFAGame()){
        ...
        if(args.len() < 1){
            ...
            if(FindAllSwitches(player) >= maxSwitches){
                NSSendInfoMessageToPlayer(player, SWITCHED_TOO_OFTEN)
                return false
            }
        ...
        if(args.len() >= 2 && args[0] == "force"){
            ...
            if(!CanFindPlayerFromSubstring(args[1])){
                NSSendInfoMessageToPlayer( player, CANT_FIND_PLAYER_FROM_SUBSTRING + args[1] )
                return false
            }
            ...
            NSSendInfoMessageToPlayer(player, fullPlayerName + SWITCH_ADMIN_SUCCESS)
            NSSendInfoMessageToPlayer(GetPlayerFromName(fullPlayerName), SWITCHED_BY_ADMIN)
...
void function SwitchPlayer(entity player, bool force = false){
    ...
    if(player.GetTeam() == TEAM_IMC){
        ...
        if(!CanPlayerSwitch(player, imcPlayerAmount, militiaPlayerAmount) && !force){
            NSSendInfoMessageToPlayer( player, SWITCH_TOO_MANY_PLAYERS )
            return
        }
        SetTeam(player, TEAM_MILITIA)
        SetPlayerModelAndSkins(player, playerModel, playerSkin, playerCamo, playerDecal)
        NSSendInfoMessageToPlayer(player, SWITCH_SUCCESS)
        return
    }
    ...
    if(player.GetTeam() == TEAM_UNASSIGNED){
        ...
        SetPlayerModelAndSkins(player, playerModel, playerSkin, playerCamo, playerDecal)
        NSSendInfoMessageToPlayer( player, SWITCH_FROM_UNASSIGNED )
        return
    }
}

void function SetPlayerModelAndSkins(entity player, asset model, int skin, int camo = 0, int decal = 0){
