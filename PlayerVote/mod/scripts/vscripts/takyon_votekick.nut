...
bool function CommandKick(entity player, array<string> args){
    if(!IsLobby()){
        ...
        if(fullPlayerName == player.GetPlayerName()){
            NSSendInfoMessageToPlayer( player, CANT_KICK_YOURSELF )
            return false
        }
        ...
        if(args.len() >= 2 && args[1] == "force"){
            ...
            NSSendInfoMessageToPlayer( player, KICKED_PLAYER + fullPlayerName )
            return true
        }

        // Check if enough people are online to initiate a vote
        if(GetPlayerArray().len() < minimumOnlinePlayers){
            NSSendInfoMessageToPlayer( player, NOT_ENOUGH_PLAYERS_ONLINE_FOR_KICK )
            return false
        }
        ...
        if(playerKickVoteYesNames.len() == 0){ // no vote going yet
            ...
            NSSendInfoMessageToPlayer( player, ALREADY_VOTE_GOING + fullPlayerName + HOW_TO_KICK )
        }
    }
    return true
}
...
bool function CommandYes(entity player, array<string> args){
    if(playerVotedForKick == ""){
        NSSendInfoMessageToPlayer( player, NO_VOTE_GOING )
        return false
    }
...
void function CheckIfEnoughKickVotes(){
    ...
    if(moreYesThanNo && enoughPeopleVoted){
        ...
        // notify
        for(int i = 0; i < GetPlayerArray().len(); i++){
            NSSendInfoMessageToPlayer(GetPlayerArray()[i], KICKED_PLAYER + playerVotedForKick)
        }
