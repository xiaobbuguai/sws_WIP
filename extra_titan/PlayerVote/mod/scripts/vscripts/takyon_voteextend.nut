    ...
    if(!IsLobby()){
        ...
        if(args.len() == 1 && args[0] == "force"){
            ...
            for(int i = 0; i < GetPlayerArray().len(); i++){
                NSSendInfoMessageToPlayer(GetPlayerArray()[i], ADMIN_EXTENDED)
        ...
        if(hasMapBeenExtended && !extendMapMultipleTimes ){
            NSSendInfoMessageToPlayer(player, MAP_CANT_BE_EXTENDED_TWICE)
            return false
        }
        ...
        if(!PlayerHasVoted(player, playerExtendVoteNames)){
            ...
            for(int i = 0; i < GetPlayerArray().len(); i++){
                if(playerExtendVoteNames.len() > 1) // semantics
                    NSSendInfoMessageToPlayer(GetPlayerArray()[i], playerExtendVoteNames.len() + MULTIPLE_EXTEND_VOTES)
                else
                    NSSendInfoMessageToPlayer(GetPlayerArray()[i], playerExtendVoteNames.len() + ONE_EXTEND_VOTE)
			}
        }
        else {
...
void function CheckIfEnoughExtendVotes(bool force = false){
    ...
    if(playerExtendVoteNames.len() >= (1.0 * GetPlayerArray().len() * extendVotePercentage) || force) {
        ...
        // message everyone
        for(int i = 0; i < GetPlayerArray().len(); i++){
            NSSendInfoMessageToPlayer(GetPlayerArray()[i], MAP_EXTENDED)
        }
