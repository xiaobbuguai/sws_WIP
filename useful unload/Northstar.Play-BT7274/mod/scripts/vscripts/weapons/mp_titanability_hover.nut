...
var function OnWeaponPrimaryAttack_TitanHover( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	if ( flyer.IsPlayer() )
		...
		if ( IsValid( soul ) && SoulHasPassive( soul, ePassives.PAS_NORTHSTAR_FLIGHTCORE ) )
			...
			horizontalVelocity = 275
		thread FlyerHovers( flyer, soundInfo, 4.5, horizontalVelocity )
	#endif
...
void function FlyerHovers( entity player, HoverSounds soundInfo, float flightTime = 4.5, float horizVel = 200.0 )
{
	...
	if ( IsValid( soul ) ){
	damageReduction = StatusEffect_AddEndless(soul, eStatusEffect.damage_reduction, 0.5)
	}
	...
	OnThreadEnd(
		function() : ( activeFX, player, soundInfo, damageReduction)
		{
			if ( IsValid( player ) )
			{
				entity soul = player.GetTitanSoul()
