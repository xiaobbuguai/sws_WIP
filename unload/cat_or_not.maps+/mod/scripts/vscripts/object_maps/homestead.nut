...
void function EntitiesDidLoad()
{
    ...
    //CreateSimpleButton( <5279, 1716, 76>, <0, 0, 0>, "to summon viper", callback_ValidateVoperRequest , 1.0 )
...
void function SarahDefenseThink( int team )
{
    ...
    foreach ( entity player in GetPlayerArray() )
    Chat_ServerPrivateMessage( player, "[莎拉] 該死，小b派毒蛇蛇穿了我們的飛船，鐵馭們，我需要你們的幫助", false )
    ...
    OnThreadEnd(
		function() : ( ship, sarah )
		{
            ...
            foreach ( entity player in GetPlayerArray() )
                NSSendInfoMessageToPlayer( player, "莎拉趨勢，在座各位都有責任" )
		}
	)
    ...
    foreach ( entity player in GetPlayerArray() )
    {
        if ( player.GetTeam() == team )
            NSSendInfoMessageToPlayer( player, "保護莎拉·布裏格斯指揮官！別讓xiaob得逞了！" )
        else
            NSSendInfoMessageToPlayer( player, "爲了xiaob！攻擊莎拉·布裏格斯指揮官！" )
    }
    ...
    {
        if ( player.GetTeam() == team )
            NSSendInfoMessageToPlayer( player, "保護莎拉·布裏格斯指揮官！直到消滅所有敵人，別讓xiaob得逞了！" )
    }
